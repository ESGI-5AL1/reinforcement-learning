...
# crates_coordinate_list = [[0, 96], [0, 160], [0, 224], [0, 288], [0, 352], [256, 96], [512, 96], [512, 160],[720, 96], [720, 160],[720,200]]
...
ENEMIES_POSITIONS = [
    ...
    (1000, 128),  # Premier tiers
    (1600, 128),  # Milieu
    (2400, 128),  # Après checkpoint


    (1296, 296),  # Sur plateforme 1200,1480,400
    (2560, 396),  # Sur plateforme 2400,2600,300
    (3100, 396)  # Sur plateforme 3000,3200,300
]
...
def setup_coins(game):
    ...
    for start_x, end_x, height in plateform_coordinate_list[1:]:
        place_multi_coins_tiles(game, start_x, end_x, 128, ":resources:images/items/gold_1.png", COIN_SCALING,
                                height + 64)
